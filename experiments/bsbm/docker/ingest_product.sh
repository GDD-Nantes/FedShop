...
docker exec bsbm-product-only /usr/local/virtuoso-opensource/bin/isql-v "EXEC=ld_dir('/usr/local/virtuoso-opensource/share/virtuoso/vad/', 'product.nq', 'http://example.com/datasets/default');" >> /dev/null
