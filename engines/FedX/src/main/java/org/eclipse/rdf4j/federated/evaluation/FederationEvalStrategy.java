...
public abstract class FederationEvalStrategy extends StrictEvaluationStrategy {
	...
	public TupleExpr optimize(TupleExpr expr, EvaluationStatistics evaluationStatistics,
		...
		long sourceSelectionTimeStart = System.currentTimeMillis();
		Set<Endpoint> relevantSources = performSourceSelection(members, cache, queryInfo, info);
		long sourceSelectionTimeEnd = System.currentTimeMillis();
		long sourceSelectionTime = sourceSelectionTimeEnd - sourceSelectionTimeStart;
		org.example.FedX.CONTAINER.setSourceSelectionTime(sourceSelectionTime);
		...
		long planningTimeStart = System.currentTimeMillis();
		optimizeJoinOrder(query, queryInfo, info);
		long planningTimeEnd = System.currentTimeMillis();
		long planningTime = planningTimeEnd - planningTimeStart;
		org.example.FedX.CONTAINER.setPlanningTime(planningTime);
