...
public class SourceSelection {
	...
	public SourceSelection(List<Endpoint> endpoints, SourceSelectionCache cache, QueryInfo queryInfo) {
        ...
        FedX.CONTAINER.setSourceSelection(this);
	}
    ...
    /**
     * Dispatch method to either default or forced source selection 
     * @param stmts
     */
	...
	public void doSourceSelectionDefault(List<StatementPattern> stmts) {
        ...
        FedX.CONTAINER.setStmtToSources(this.stmtToSources);
	}
    ...
    public void doSourceSelectionForce(List<StatementPattern> stmts) {
		...
		for (StatementPattern stmt : stmts) {
			...
			log.info("tp" + stmtId +": " + stmt.toString());

			stmtToSources.put(stmt, new ArrayList<>());

			SubQuery q = new SubQuery(stmt, queryInfo.getDataset());

            Set<String> forced_endpoints = FedX.CONTAINER.getTriplePatternSources().get(stmtId);

			// check for each current federation member (cache or remote ASK)
            for (String forced_endpoint: forced_endpoints ) {
                for (Endpoint e: endpoints) {
                    if (e.getId().equals(forced_endpoint)){
                        StatementSourceAssurance a = cache.getAssurance(q, e);
                        if (a == StatementSourceAssurance.HAS_REMOTE_STATEMENTS) {
                            StatementSource statement_source = new StatementSource(forced_endpoint, StatementSourceType.REMOTE);
                        	addSource(stmt, statement_source);
                        } else if (a == StatementSourceAssurance.POSSIBLY_HAS_STATEMENTS) {
                            remoteCheckTasks.add(new CheckTaskPair(e, stmt, queryInfo));
                        } else if (a == StatementSourceAssurance.NONE) {
                            // cannot provide any statements
