...
def run_benchmark(ctx: click.Context, eval_config, engine_config, query, result, stats, source_selection, batch_id):
    ...
    with open(properties, "r") as properties_file:
        ...
        for i in range(len(lines)):
            if lines[i].startswith("query.directory"):
                lines[i] = re.sub(r'query.directory=.+',r'query.directory='+str(work_dir)+str(query).split("injected.sparql")[0][:-1], lines[i])
    ...
    cmd = f'./{script} {properties}'

    print("=== SPLENDID ===")
    print(cmd)
    print("================")

    splendid_proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    input_provenance_file = f"{work_dir}engines/SPLENDID/eval/{provenance_stat_to_modif}"
    output_provenance_file = f"{work_dir}engines/SPLENDID/eval/{provenance_stat_to_modif}"

    data_transform(input_provenance_file, output_provenance_file)

    ideal_ss = f"engines/SPLENDID/eval/{provenance_stat_to_modif}"
    ...
    def write_empty_stats():
        if stats != "toIgnore":
            with open(stats, "w") as fout:
                ...
                basicInfos = re.match(r".*/(\w+)/(q\w+)/instance_(\d+)/batch_(\d+)/(\w+)/results", result)
...
@click.option("--configfile", type=click.Path(exists=True, file_okay=True, dir_okay=False), default="experiments/bsbm/config.yaml")
def generate_config_file(datafiles, outfile, endpoint, configfile):
    splended_config = load_config(configfile)["evaluation"]["engines"]["splendid"]

    with open(splended_config["config"],'w+') as outputfile:
        outputfile.write("################################################################################\n")
        ...
        outputfile.write("\t\t\t\tfed:useTypeStats true ;\n")
        outputfile.write("\t\t\t] ;\n")
        outputfile.write("\n")
        ...
        outputfile.write("\t\t\t\tfed:groupBySameAs true ;\n")
        outputfile.write("\n")
        outputfile.write("\t\t\t\tfed:useBindJoin true ;\n")
    ...
    for file in datafiles:
        ...
        with open(str(splended_config["data_summaries"])+str(file).split('/')[-1].split('.')[0]+'.n3','w+') as outputfile:
            outputfile.write("")
        with open(file) as inputfile:
            ...
            for i in map_pred_type.keys():
                ...
                if("P:" in value):
                    ...
                    type_count+=1
            with open(str(splended_config["data_summaries"])+str(file).split('/')[-1].split('.')[0]+'.n3','a') as outputfile:
                outputfile.write("@prefix void: <http://rdfs.org/ns/void#> .\n")
                outputfile.write("\n")
...
#@cli.command()
#@click.argument("input_provenance_file", type=click.Path(exists=True, file_okay=True, dir_okay=True))
#@click.argument("output_provenance_file", type=click.Path(file_okay=True, dir_okay=True))
